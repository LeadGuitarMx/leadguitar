...
if(!$("#ablockercheck").is(":visible") && $('#ablockercheck').length){$("#ablockermsg").prepend("<div class='modal' id='ayudanos' tabindex='-1' role='dialog'><div class='modal-dialog' role='document'><div class='modal-content bg-dark text-light'><button class='close closeframe' data-dismiss='modal' type='button' aria-label='Close'><span aria-hidden='true'>&times;</span></button><h5 class='modal-title'>Hola ! 🙂</h5> <p>Por favor considera <span class='text-danger'>desactivar el Blockeador de Publicidad para éste sitio </span>.</p><p>Ya que sin la publicidad no podemos mantenernos en linea.</p><p>Te prometo que <span class='text-success'>no verás mucha publicidad ni popups molestos</span>.</p><p>De antemano muchas gracias por ayudarnos. 😉 Saludos ! </p><div class='text-right mt-20'><button class='btn btn-primary closeframe' data-dismiss='modal' type='button'>OK entiendo</button></div></div></div>").show();halfmoon.toggleModal('ayudanos');}
/* Slider Unblock */
